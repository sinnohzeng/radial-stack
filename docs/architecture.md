...
│   ├── style.css              样式
│   └── public/                Cloudflare Pages 静态配置
│       ├── _headers           安全头 + 缓存策略
│       └── _redirects         SPA fallback
├── config/
│   └── default.json           默认配置（SSOT）
├── vite.config.js         <-- Vite 构建配置（root: web/）
├── tests/                 <-- vitest 测试（204 个）
...
本项目的渐变实现参照了 Justin Jay Wang 的 SVG 渐变方法论，核心思路是**多层径向渐变叠加**产生丰富的色彩变化。

### 算法流程

1. **选色**：从配色方案中随机挑选 4 种主色（`selectPrimaryColors`）
2. **定义渐变**：为每种主色创建一个 `<radialGradient>`，焦点位置（fx/fy）完全随机化，部分渐变使用 3 个色标（通过 OKLCH 中间色增强过渡）
3. **叠加层**：生成 8 层（默认） `<rect>` 元素，每层引用一个渐变定义并应用独立变换
4. **后处理**：默认高斯模糊（blur=3）柔化边缘 + CSS `filter:saturate()` 增强色彩饱和度

### 多层渐变叠加

每个渐变定义被多层 `<rect>` 循环引用（`i % gradientIds.length`），默认 8 层（可配置 1-30），每层具有独立的变换参数和不透明度。外层偏透明（0.4-0.7），内层偏实（0.7-1.0），产生自然的深度感。
...
Web 预览使用 Vite 作为构建工具，解决浏览器无法解析 npm bare specifier（如 `import seedrandom from 'seedrandom'`）的问题。

```
vite.config.js
├── root: 'web'              ← 入口目录，index.html 在此
├── build.outDir: '../dist'  ← 构建输出到项目根 dist/
└── test.root: '.'           ← Vitest 在项目根目录查找测试
```

**关键设计决策：** `root: 'web'` 确保 `dist/index.html` 在根路径（而非 `dist/web/index.html`）。`../src/core/` 的相对导入仍然有效，因为 Rollup 打包器跟随相对路径，不受 root 限制。

### Cloudflare Pages 部署

- `web/public/_headers`：安全头（X-Content-Type-Options、X-Frame-Options）+ 构建资产长期缓存（Vite content hash）
- `web/public/_redirects`：SPA fallback（`/* → /index.html 200`）
- `.node-version`：锁定 Node 18 用于 Cloudflare Pages 构建环境
